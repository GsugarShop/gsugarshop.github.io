...
const scrollProfileBtn = document.getElementById('scrollProfileBtn');
...
function renderUserOrders(email) {
  ...
  if (!matched.length) {
    userOrdersPanel.innerHTML = '<p class="empty-note">No orders found for this email yet. Place an order and check back later.</p>';
    return;
  }
  ...
  userOrdersPanel.innerHTML = matched.map(order => `
    <article class="order-card">
      ...
      <div class="order-actions">
        ...
        ${order.status === 'email' ? `<a class="btn btn-outline" href="mailto:${order.customerEmail}?subject=${encodeURIComponent('Your GSugar order details')}">Open inbox</a>` : ''}
      </div>
    </article>
  `).join('');
}
